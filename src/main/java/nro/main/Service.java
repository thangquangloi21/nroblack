            ...
            if (item != null)
                if (actionBuy == 0){
                    if(item.id == 457){
                        ...
                        Service.gI().SaleDone(player, typeBuy, "Bạn có muốn bán\n"+player.ItemBag[indexBuy].template.name +"\nVới giá 1 vàng không?" , indexBuy);
                    }
                }
                else if(actionBuy == 1){
                    if(item.id == 457){
                        if(((long)player.vang + 500000000) > 2000000000L){
                            player.sendAddchatYellow("Không được quá 2 tỉ vàng");
                        }
                         else {
                            player.updateQuantityItemBag(indexBuy, 1);
                            Service.gI().updateItemBag(player);
                            long _vang = ((long) player.vang + 500000000) > 2000000000L ? 2000000000L : ((long) player.vang + 500000000);
                            player.vang = _vang;
//                        player.vang += 500000000;
                            Service.gI().buyDone(player);
                            player.sendAddchatYellow("Bán vật phẩm thành công");
                        }
                    }
                    else {
                        player.updateQuantityItemBag(indexBuy,1);
